...
export default {
            ...
            this.collector.subscribe(
              {
                ...
                validateCb: () => {
                  return new Promise( ( resolve, reject ) => {
                    validator.
                      validate( { [ this.field ]: this.value } ).
                      then( resolve.bind( null, this.field ) ).
                      catch( ( errors ) => {
  ...
  render( h ) {
    return h(
      'div',
      {
        class: [ 'scm-validate-wrap' ],
        attrs: {
          'data-validate-field': this.field
        }
      },
      [
        h(
          'label',
          {
            ...
            class: [
              'scm-validate-label',
              this.rules.some( ( rule ) => rule.required ) && 'scm_required'
            ].filter( Boolean )
          },
        ...
        h(
          'div',
          {
            class: [ 'scm-validate-content' ]
          },
          [ this.$scopedSlots?.default(),
            this.validateData.isPass && h( 'div', {
              class:[ 'scm-validate-error', `scm-validate-error_${ this.field }` ]
            },
    ...
    /* return <div class='scm-validate-wrap' data-validate-field={}>
      <label
        for={this.field}
        class={`scm-validate-label ${ this.rules.some( rule => rule.required ) ?
           'scm_required' : '' }`}
      >
        {
          this.label ? this.label : this.$scopedSlots?.label()
